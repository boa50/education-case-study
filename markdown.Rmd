...
large group of countries on the same group. So with this in mind, we check that
there is a lot of room to improvement.

### What influence does money spent in education have?

To get started on which aspects could be improved to get a better school
enrollment level, we analyzed data about GDP spent in education.
...
goods and services in a country during a certain period". So I tought that it
could be a good starting point on our analysis.

The first step, as always was to group and clean the data about this indicator,
that represents the percentage of the country's GDP spent in education.
...
We checked the number of NA values to all indicator as it is a constant 
difficulty using this dataset, and to this case we used the same strategy as the
...
The chart above shows that there is little variation on the percentage of GDP
spent in education among all countries during the last 10 years. So, we conclude
that the GDP spent in education is not relevant to our analysis.

But wait! This is only part of the analysis on GDP, because it covers only the
variation in percentage and countries could have a huge difference on absolute
values of GDP and GDP per capita. So we tried to extract this type of information
and combine it with the data about spendings in education.
...
df_gdp <- df_cleaned %>%
  ...
  mutate(year=gsub('x', '', .$year)) %>%
  group_by(country_code, indicator_code) %>%
  # Using the strategy to fill NA values to get similar number of coutries' values
...
Now we can see that the difference on absolute values of GDP per capita is very
...
need to increase our investments on education to get better results.

### Does children with jobs have more or less oportunities?

After that, the impacts of children employment were studied to verify if there
is some correlation with children enrollment in schools or not.
...
to show the variations. This happened because the number of NA values on it is
so huge that was preferable to get only the last year with valid information.

To not get an outdated information, we chose to pick the last available data
...
those with higher incomes showing a good correlation with the data about students
enrollment.
...
On the last step, we analyzed data about income share between population groups.
...
```{r Checking NA values in income share, echo=FALSE, results='hold'}
...
of information through years.

With the histogram below we could see that the variations on the values are very
...
On the above chart we could get that in Brazil, the differences of income share
between population groups is very big, bigger that any other country analyzed.
These differences could increase the difficulties that poorer people have to
sustain their lives, making it harder to them to improve their education level.
...
NA values through it and with this we needed to make some assumptions on how
...
Other important aspects that could be noted is that World Bank could be considered
a secondary source of the data on some indicators (and a good one) and some 
mistakes could have been done during the process of obtaining the data. So getting
data from primary sources could direct us to bit different results.

## Final toughts

On this analysis we checked some indicators that we would believe that could 
increase children enrollment in schools, but it is the most important part in
education? Maybe children that study only at home could get better results than
those that go to school. If this is true our whole analysis could be changed :)

Other thing that we have to pay attention when working with data is that
...
I end here with a last quote: "*Think differently and have a better life.*"